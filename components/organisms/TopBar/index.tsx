...
export function TopBar() {
  ...
  const SaveIcon =
    saveFlash === "saved" ? Check : saveFlash === "error" ? AlertCircle : Save;
  const saveFlashClass = cn(
    saveFlash === "saved" && style.saveFlashSaved,
    saveFlash === "error" && style.saveFlashError,
  );
    ...
    <header className={style.header}>
      ...
      <Link href="/?stay=1" className={style.brand}>
        <div className={style.brandInner}>
          <div className={style.logo}>
            <Workflow size={14} className={style.logoIcon} />
          </div>
      ...
      <div className={style.actions}>
        ...
        <Separator
          ...
          className={cn(style.vSep, style.desktopOnly)}
        />
        <ToolbarButton
          icon={SaveIcon}
          tooltip={saveFlash === "saved" ? "Saved!" : "Save (Ctrl+S)"}
          onClick={handleSave}
          className={cn(style.desktopOnly, saveFlashClass)}
        />
        <Separator
          orientation="vertical"
          className={cn(style.vSepWide, style.desktopOnly)}
        />
        ...
        <ToolbarButton
          ...
          tooltip="Save"
          onClick={handleSave}
          className={cn(style.mobileOnly, saveFlashClass)}
        />
        <ToolbarButton
          icon={Download}
          tooltip="Export JSON"
