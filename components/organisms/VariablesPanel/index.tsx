...
export function VariablesPanel() {
  ...
  // Mobile has no right dock — keep the slide-over.
  if (isMobile) {
    if (!variablesPanelOpen || typeof document === "undefined") return null;
  ...
  return (
    <motion.aside
      ...
      animate={{ flexGrow: variablesPanelOpen ? 1 : 0, opacity: variablesPanelOpen ? 1 : 0 }}
      transition={{ type: "spring", stiffness: 320, damping: 32 }}
      className={cn(style.panel, variablesPanelOpen && style.panelOpen)}
      inert={!variablesPanelOpen}
    >
      {content}
    </motion.aside>
  );
